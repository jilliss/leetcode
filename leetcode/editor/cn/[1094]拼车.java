...
class Solution1094 {
    public boolean carPooling(int[][] trips, int capacity) {
        ...
        for (int[] trip : trips) {
            ...
            int end = trip[2];
            // if (start == 0) {
            zeroArr[start] += numPassengers;
            // } else {
            //     zeroArr[start - 1] += numPassengers;
            // }
            // if (end != 1){
            zeroArr[end] -= numPassengers;
            // }
        }
        originalArr[0] = zeroArr[0];
        int sum = 0;
        for (int j : zeroArr) {
            sum += j;
            // originalArr[i] = zeroArr[i] + originalArr[i - 1];
            if (sum > capacity) {
                return false;
            }
        }
    ...
    //     new Solution().carPooling(new int[][]{
    //             // new int[]{4, 3, 4}, new int[]{3, 2, 4},
    //             // new int[]{1, 8, 9}, new int[]{7, 2, 5},
    //             new int[]{9, 0, 1}, new int[]{3, 3, 7},
    //             // new int[]{8, 3, 9},
    //     }, 4);
    // }
